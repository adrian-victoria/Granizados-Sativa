  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO productos (nombre, descripcion, precio, categoria_id, stock, stock_minimo, tiene_licor, imagen_url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [nombre, descripcion || null, precio, categoria_id, stock || 0, stock_minimo || 5, tiene_licor === 'true' ? 1 : 0, imagen_url]
    );
...
async function editar(req, res) {
  ...
  try {
    ...
    if (tiene_licor !== undefined)  { campos.push('tiene_licor = ?');  valores.push(tiene_licor === 'true' ? 1 : 0); }
